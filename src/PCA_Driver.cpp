...
#ifdef PCA9685_LINUX
int PCA9685::get_kI2Address(){
    return kI2CAddress;
}

uint8_t PCA9685::get_kI2CBus(){
    return kI2CBus;
}

void PCA9685::set_kI2CBus(uint8_t bus){
    kI2CBus = bus;
}
#endif
...
int PCA9685::getError(){
    return error;
}


uint8_t PCA9685::getPrescale(){
    return prescale_;
}
...
double PCA9685::getPeriodSlice(){
    return period_slice_; 
}

double PCA9685::getClock(){
    return cpuClock_;
}
...
void PCA9685::setPWMFrequency ( float frequency ) {
    ...
    //int prescale = (int)(27500000.0f / (4096 * rangedFrequency) - 0.5f) ; // 10% higher clock
    // For debugging
...
void PCA9685::setExtClk() {
  ...
  writeByte(PCA9685_MODE1, newmode); // go to sleep, turn off internal oscillator
...
void PCA9685::setPWMBias(int bias){
    pwmOnBias_ = bias;
}

int PCA9685::getPWMBias(){
    return pwmOnBias_;
}
